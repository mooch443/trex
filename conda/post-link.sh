if [ "$(uname -p)" == "arm" ]; then
	...
	echo "    conda activate $(basename ${PREFIX}) && conda install -c apple -y tensorflow-deps==2.7.0 && python -m pip install tensorflow-macos==2.7.0 tensorflow-metal==0.3.0"  >> $PREFIX/.messages.txt;
	echo "============ /TRex ==========" >> $PREFIX/.messages.txt;
fi
