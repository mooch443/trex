...
if [ "$(uname -p)" == "arm" ]; then
    ...
    { python -m pip install 'torch>=2.0.0,<2.9.0' 'torchvision<0.24.0' torchmetrics tqdm 'opencv-python>=4,<5' 'ultralytics>=8.3.0,<9' numpy==${numpy} dill 2>&1; } >> $OUT_STREAM;
elif [ "$(uname)" == "Darwin" ]; then
    echo "macOS detected, installing packages..." >> $OUT_STREAM
    { python -m pip install 'torch>=2.0.0,<2.9.0' 'torchvision<0.24.0' torchmetrics tqdm 'opencv-python>=4,<5' 'ultralytics>=8.3.0,<9' numpy==${numpy} dill 2>&1; } >> $OUT_STREAM
else
    echo "Linux architecture detected, installing packages..." >> $OUT_STREAM
    { python -m pip install torchmetrics 'torch>=2.0.0,<2.9.0' 'torchvision<0.24.0' tqdm 'opencv-python>=4,<5' 'ultralytics>=8.3.0,<9' numpy==${numpy} "dill" --index-url https://download.pytorch.org/whl/cu118 --extra-index-url https://pypi.org/simple 2>&1; } >> $OUT_STREAM
fi
