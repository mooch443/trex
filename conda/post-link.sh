...
if [ "$(uname -p)" == "arm" ]; then
    ...
    { python -m pip install 'torch>=2.0.0,<2.9.0' 'torchvision>=0.15.1,<0.24.0' torchmetrics tqdm 'opencv-python>=4,<5' 'ultralytics>=8.3.0,<9' numpy==${numpy} dill 2>&1; } >> $OUT_STREAM;
...
PY
    ...
    if [ "${gpu_check}" = "True" ]; then
        extra_index_args=(--index-url https://download.pytorch.org/whl/cu118 --extra-index-url https://pypi.org/simple)
    fi
    { python -m pip install torchmetrics 'torch>=2.0.0,<2.9.0' 'torchvision>=0.15.1,<0.24.0' tqdm 'opencv-python>=4,<5' 'ultralytics>=8.3.0,<9' numpy==${numpy} dill "${extra_index_args[@]}" 2>&1; } >> $OUT_STREAM
fi
