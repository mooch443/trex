...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	Pause the analysis (``,`` key, this may take a few seconds). The matching algorithm (can be changed in :param:`match_mode`) has diffculty separating individuals into distinct cliques of neighboring individuals, or there are simply too many of them - which only happens if you changed the :param:`match_mode` from `automatic` to `tree`. This could be because your video contains too many trackable objects and no limit on the number of individuals has been set (:param:`track_max_individuals`), or there are significant time-jumps in the video. If the number of individuals should be much lower than detected, check your :param:`track_threshold`/:param:`track_size_filter` settings. If that does not fix anything, check your settings for :param:`track_max_speed`, which controls the size of the neighbourhood to be considered during matching, and reduce it until there are no further warnings. Alternatively, change the :param:`match_mode` to automatic, which should get rid of these problems.
