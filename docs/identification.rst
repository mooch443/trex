...
You also have to set a fixed number of individuals by setting :param:`track_max_individuals` manually, or relying on the automatic detection. This value has to be either appended to the command-line, or saved in the video's settings file to be applied before the program is started.

Once you are happy with the tracking results, click on menu -> visual identification. This gives you a short description that you should probably read. Now, click "Start". Keep in mind that this will remove all previously added :param:`manual_matches` and retrack the entire video - automatically applying the generated identity predicitions.
