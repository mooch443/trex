PWD=$(pwd)/../../videos
tgrabs -d "${PWD}" -i 8guppies_20s.mp4 \
     -o test -threshold 9 -average_samples 100 -averaging_method mode -meta_real_width 30 -exec "${PWD}/test.settings" \
     -enable_live_tracking -auto_no_results -auto_no_tracking_data false -nowindow
...
for f in `ls ${PWD}/data/test_fish*.npz`; do
    ...
    MD5_CURRENT="$(md5 -q ${PWD}/data/${f}/X.npy) $(md5 -q ${PWD}/data/${f}/SPEED#wcentroid.npy)"
    MD5_COMPARE="$(md5 -q ${PWD}/compare_data/raw/${f}/X.npy) $(md5 -q ${PWD}/compare_data/raw/${f}/SPEED#wcentroid.npy)"

    echo "Checking '$f'... $MD5_COMPARE | $MD5_CURRENT"

    if [ "$MD5_COMPARE" != "$MD5_CURRENT" ]; then
        echo "md5 sum for $f differs from baseline"
        exit 1
    fi
