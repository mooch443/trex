...
#ifndef TREX_TEST_FOLDER
#error "TREX_TEST_FOLDER must be defined"
#endif
...
TEST(TrexDetectionModelPythonTest, RunsRfDetrAdapterUnitTests) {
    const std::string python = TREX_PYTHON_EXECUTABLE;
...
TEST(TrexDetectionModelPythonTest, RunsYoloSpecificRuntimePolicyTests) {
    const std::string python = TREX_PYTHON_EXECUTABLE;
    const std::string test_dir = TREX_DETECTION_MODEL_TEST_DIR;
...
TEST(TrexDetectionModelPythonTest, RunsDetectorBackendRealModelEndToEndTests) {
    const std::string python = TREX_PYTHON_EXECUTABLE;
    const std::string test_dir =
        (fs::path(TREX_TEST_FOLDER) / "python").string();
    ...
    const std::string command =
        "set PYTHONDONTWRITEBYTECODE=1 && " + quote_for_shell(python) +
        " -B -m unittest discover -s " + quote_for_shell(test_dir) +
        " -p test_trex_detector_backends_end_to_end.py";
#else
    const std::string command =
        "PYTHONDONTWRITEBYTECODE=1 KMP_DUPLICATE_LIB_OK=TRUE " + quote_for_shell(python) +
        " -B -m unittest discover -s " + quote_for_shell(test_dir) +
