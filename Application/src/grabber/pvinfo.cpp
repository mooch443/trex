...
ENUM_CLASS(Arguments,
           display_average, i, input, remove, repair_index, fix, quiet,save_background, plain_text, heatmap, auto_parameters, s, p, d, dir, md, opencv_ffmpeg_support, opencv_opencl_support, uniqueness, output_fps, blob_detail)

ENUM_CLASS(parameter_format_t, settings, minimal)

struct CliFlags {
    bool fix                 = false;
    bool repair_index        = false;
    bool save_background     = false;
    bool be_quiet            = false;
    bool print_plain         = false;
    bool heatmap             = false;
    bool auto_param          = false;
    bool calculate_uniqueness = false;
    bool output_fps = false;
    bool blob_detail = false;
};
...
// -----------------------------------------------------------------------------
//  Helper functions extracted from the original monolithic main()
// -----------------------------------------------------------------------------
static void initialize_runtime(int argc, char** argv);
static void parse_command_line(int argc, char** argv,
                               CliFlags& flags,
                               std::map<std::string,std::string>& updated,
                               std::vector<std::string>& remove);

static int  process_input(const CliFlags& flags,
                         const std::map<std::string,std::string>& updated,
                         const std::vector<std::string>& remove,
                         int argc, char** argv);

// -----------------------------------------------------------------------------
//  Low-level helpers for flagged actions inside process_input()
// -----------------------------------------------------------------------------
static int  handle_merge_videos(const std::vector<file::Path>& merge_videos, int argc, char** argv);
static void preload_settings_file();
static int  process_video_branch(const CliFlags& flags,
                                 const std::map<std::string,std::string>& updated_settings,
                                 const std::vector<std::string>& remove_settings);
static int  process_results_branch(const std::map<std::string,std::string>& updated_settings,
                                   const std::vector<std::string>& remove_settings,
                                   const file::Path& input);
static void handle_heatmap(const CliFlags& flags);
static void handle_write_settings(const CliFlags& flags);
static void handle_save_background(const CliFlags& flags, const file::Path& input, pv::File& video);
static void handle_replace_background(pv::File& video);
static void handle_repair_index(const CliFlags& flags, pv::File& video);
static void handle_fix(const CliFlags& flags, pv::File& video);
static void handle_metadata_updates(const std::map<std::string,std::string>& updated_settings,
                                    const std::vector<std::string>& remove_settings,
                                    pv::File& video);
static void handle_display_average(pv::File& video);
static void handle_output_fps(const CliFlags& flags, pv::File& video);
static void handle_blob_detail(const CliFlags& flags, pv::File& video);
static void handle_print_parameters();
                               ...
                               std::vector<std::string>& remove_settings)
{
    ...
    cmd.load_settings();
    flags.be_quiet = GlobalSettings::is_runtime_quiet();
    set_runtime_quiet(flags.be_quiet);
        ...
        if(Arguments::has(option.name)) {
            switch (Arguments::get(option.name)) {
                ...
                case Arguments::opencv_ffmpeg_support:
                    exit(handle_opencv_ffmpeg_support());
                    
                case Arguments::opencv_opencl_support:
                    exit(handle_opencv_opencl_support());
                ...
                case Arguments::quiet:
                    if(option.value)
                        flags.be_quiet = Meta::fromStr<bool>(*option.value);
                    else
                        flags.be_quiet = true;
                    
                    SETTING(quiet) = flags.be_quiet;
                    break;
                    
                case Arguments::plain_text:
                    flags.print_plain = true;
                    break;
                case Arguments::s:
                    if(option.value)
                ...
                case Arguments::repair_index:
                    flags.repair_index = true;
                    break;
                    
                case Arguments::save_background:
                    flags.save_background = true;
                    break;
                ...
                case Arguments::auto_parameters:
                    ...
                    flags.auto_param = true;
                    break;
                    
                case Arguments::uniqueness:
                    flags.calculate_uniqueness = true;
                    break;
                case Arguments::output_fps:
                    flags.output_fps = true;
                    break;
...
static void preload_settings_file() {
    ...
    if(settings_file.exists())
        GlobalSettings::load_from_file(default_config::deprecations(),
                                       settings_file.str(),
...
static int process_video_branch(const CliFlags& flags,
    ...
    if(flags.auto_param || SETTING(auto_minmax_size) || SETTING(auto_number_individuals)) {
        track::Tracker::auto_calculate_parameters(video, flags.be_quiet);
    }
    ...
    Output::Library::Init();
    set_runtime_quiet(flags.be_quiet);
    handle_heatmap(flags);
    handle_write_settings(flags);
    
    if(flags.print_plain) {
        printf("version %d\nframes %u\n", (int)video.header().version, video.length().get());
    }
    
    handle_save_background(flags, input, video);
    handle_replace_background(video);
    handle_repair_index(flags, video);
    handle_fix(flags, video);
    handle_metadata_updates(updated_settings, remove_settings, video);
    handle_display_average(video);
    handle_output_fps(flags, video);
    handle_blob_detail(flags, video);
...
static void handle_heatmap(const CliFlags& flags) {
    if(flags.heatmap) {
        ...
        results.load([bq = flags.be_quiet](const std::string& title, float percent, const std::string& text){
...
static void handle_write_settings(const CliFlags& flags) {
    if(SETTING(write_settings)) {
        ...
        if(filename.exists() && !flags.be_quiet)
                         ...
                         int argc, char** argv)
{
    ...
    if(!merge_videos.empty())
        return handle_merge_videos(merge_videos, argc, argv);

    preload_settings_file();

    file::Path input = SETTING(filename).value<file::Path>();
    if(SETTING(is_video)) {
        return process_video_branch(flags, updated_settings, remove_settings);
    } else {
        return process_results_branch(updated_settings, remove_settings, input);
    }
}

int main(int argc, char**argv) {
    initialize_runtime(argc, argv);
    ...
    parse_command_line(argc, argv,
                       flags,
                       updated_settings, remove_settings);

    return process_input(flags,
                         updated_settings, remove_settings,
                         argc, argv);
}
