...
void Recognition::load_weights(std::string postfix) {
    ...
    /*float Recognition::available_weights_accuracy(std::shared_ptr<TrainingData> data) {
        auto running = set_running(true, "available_weights_accuracy");
        std::future<bool> future;
        std::shared_ptr<float> result = std::make_shared<float>(INFINITY);
        
        {
            // try doing everything in-memory without saving it
            std::unique_lock<decltype(_data_queue_mutex)> guard(_data_queue_mutex);
            
            assert(!data->all_classes().empty());

            using py = PythonIntegration;
            Debug("Testing predictions (validating weights with %d images)", data->size());
            
            future = py::async_python_function([this, data, result]() -> bool
            {
                if(FAST_SETTINGS(manual_identities).size() > data->all_classes().size()) {
                    std::set<long_t> missing;
                    for(auto id : FAST_SETTINGS(manual_identities)) {
                        if(data->all_classes().find(id) == data->all_classes().end())
                            missing.insert(id);
                    }
                ...
                std::vector<long_t> class_vector(data->all_classes().begin(), data->all_classes().end());
                auto && [images, ids] = data->join_arrays();
                
                py::set_variable("X", images);
                py::set_variable("Y", ids);
                py::set_variable("classes", class_vector);
                
                try {
                    
                    std::string str = utils::read_file("test_predictions.py");
                    py::execute(str);
                    
                    auto py_result = py::get_variable<float>("prediction_result");
                    *result = py_result;
                    
                    Debug("Returning %d (%f)", *result != INFINITY, *result);
        ...
        _last_prediction_accuracy = -1;
        
        std::string reason;
        try {
            if(future.get()) {
                DebugCallback("Prediction successful (accuracy %.2f%%).", *result * 100);
                _last_prediction_accuracy = *result;
                return *result;
            } else
                Debug("Prediction returned false.");
        ...
        SOFT_EXCEPTION("Prediction failed ('%S').", &reason);
    }*/
