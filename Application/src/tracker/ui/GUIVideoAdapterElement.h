...
struct GUIVideoAdapterElement : public dyn::CustomElement {
    ...
    std::function<Layout::Ptr(const file::PathArray&, Base*, std::function<void(VideoInfo)>)> _create_object;
    Base *_window;
    
    std::optional<std::string> _last_path_str;
    std::optional<file::PathArray> _last_path;
    
    GUIVideoAdapterElement(Base* window,
                           std::function<Size2()> size_function,
                           std::function<void(VideoInfo)> open_callback = nullptr,
                           std::function<Layout::Ptr(const file::PathArray&, Base*, std::function<void(VideoInfo)>)> = nullptr);
