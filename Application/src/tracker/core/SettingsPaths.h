...
/// source-derived name. User-selected names and ordinary source-derived names
/// are routed through the registered `output` DataLocation, including its
/// `output_dir` and `output_prefix` rules. A sole `.pv` source instead resolves
/// to that source's absolute base path. Any trailing `.pv` is removed.
...
/// supplies the output-side candidate. Relative selected names are routed
/// through the registered `output` DataLocation; absolute selected names remain
/// absolute. If that candidate does not exist, a sole explicit `.pv` source—or
/// an extensionless source with an existing `.pv` sibling—is used as the
/// tracking input. The returned path never has a `.pv` extension. Throws when
/// no usable PV exists.
