...
//#endif
        ...
        CONFIG("gui_blob_label", std::string("{if:{dock}:{name} :''}{if:{active}:<a>:''}{real_size}<white>cm²</white>{if:{split}: <gray>split</gray>:''}{if:{tried_to_split}: <orange>split tried</orange>:''}{if:{prediction}: {prediction}:''}{if:{instance}: <gray>instance</gray>:''}{if:{dock}:{if:{filter_reason}: [<gray>{filter_reason}</gray>]:''}:''}{if:{active}:</a>:''}{if:{category}: {category}:''} {round:{*:{p_for_primary}:100}}%"), "This is what the graphical user interfaces displays as a label for each blob in raw view. Replace this with {help} to see available variables.");
