...
bool execute_settings_file(const file::Path& source, AccessLevelType::Class level, const std::vector<std::string>& exclude) {
    ...
    void get(sprite::Map& config, GlobalSettings::docs_map_t& docs, std::function<void(const std::string& name, AccessLevel w)> fn)
    {
        ...
        CONFIG("cm_per_pixel", Float2_t(1), "The ratio of `meta_real_width / video_width` that is used to convert pixels to centimeters. Will be automatically calculated based on a meta-parameter saved inside the video file (`meta_real_width`) and does not need to be set manually.");
...
//#endif
        ...
        CONFIG("segment_size_filter", BlobSizeRange(), "During conversion (using background subtraction) objects outside this size range will be filtered out. If empty, all objects will be accepted.");
        CONFIG("track_size_filter", BlobSizeRange(), "Blobs below the lower bound are recognized as noise instead of individuals. Blobs bigger than the upper bound are considered to potentially contain more than one individual. You can look these values up by pressing `D` in TRex to get to the raw view (see `https://trex.run/docs/gui.html` for details). The unit is #pixels * (cm/px)^2. `cm_per_pixel` is used for this conversion.");
